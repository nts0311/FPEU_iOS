...
public class SheetTransition: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    public func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if presenting {
            guard let presenter = transitionContext.viewController(forKey: .from), let sheet = transitionContext.viewController(forKey: .to) as? VDSBottomSheetVC else {
            ...
            UIView.animate(
                ...
                animations: {
                    if self.options.shrinkPresentingViewController {
                        ...
                        let topSafeArea = HelperFunction.heightSafeAreaTop()
            ...
            let sheet = transitionContext.viewController(forKey: .from) as? VDSBottomSheetVC else {
                transitionContext.completeTransition(true)
                return
            }
    ...
    func setPresentor(percentComplete: CGFloat) {
        guard self.options.shrinkPresentingViewController, let presenter = self.presenter else {
            return
        }
